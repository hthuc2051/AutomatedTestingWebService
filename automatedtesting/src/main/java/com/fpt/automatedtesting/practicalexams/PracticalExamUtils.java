...
public class PracticalExamUtils {
    ...
    public static String removeNullOrBlankElements(String lineOfCode,String language)
    {
        ...
        if(language.equals(PathConstants.LANGUAGE_JAVA))
        {
            try{
                ...
                for (String line:removedNull) {
                    ...
                    if(line.length()>= 6)
                    {
                        if(line.substring(0,6).contains(PathConstants.IMPORT_JAVA))
                        {
        ...
        }else if(language.equals(PathConstants.LANGUAGE_CS))
        {
            try{
                List<String> removeList = new ArrayList<>(Arrays.asList(removedNull));
                for (String line:removedNull) {
                    ...
                    if(line.length() >= 5) {
                        if(line.substring(0,5).contains(PathConstants.IMPORT_CS))
                        {
        ...
        else if(language.equals(PathConstants.LANGUAGE_C))
        {
            try{
                List<String> removeList = new ArrayList<>(Arrays.asList(removedNull));
                for (String line:removedNull) {
                    //System.out.println(line.substring(0,5));
                    //System.out.println(line.substring(7,line.length()));line.substring(0,8);

                    if(line.length() >= 8)
                    {
                        if(line.substring(0,8).contains(PathConstants.IMPORT_C))
                        {
    ...
    public static void searchRepo(GitHub gitHub,String lineOfCode,String language) throws IOException {
        try
        {
    ...
    public static void writeToFile(String fileReportPath,String queryCode,List<String> resultPage) throws IOException {
        try {
            String [] resultConverted = resultPage.toArray(new String[0]);
            FileWriter writer = new FileWriter(PathConstants.GITHUB_LOG_PATH, true);
    ...
    // Use this method
    public static void logGithub(String filePath) throws IOException {
        try{
            ...
            {
                language = PathConstants.LANGUAGE_JAVA;
            }else if(filePath.contains(".cs"))
            {
                language=PathConstants.LANGUAGE_CS;
            }else if(filePath.contains(".c"))
            {
                language=PathConstants.LANGUAGE_C;
            }
