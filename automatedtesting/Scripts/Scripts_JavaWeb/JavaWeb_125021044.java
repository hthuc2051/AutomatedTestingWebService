...
class TestwebApplicationTests {
    public static String questionPointStr = "checkConnection:2-checkLoginDAO:1-showAllDAO:1-deleteDAO:1-checkLogin:1-checkWelcome:1-showAllUI:1-deleteUI:1-logOut:1";
...
@Order(5) 
public void checkLogin(){if( !isLogin ){assertTrue( false );}else{if( driver != null ){driver.get( "http://localhost:8080/login.html" );driver.findElement(By.name( "txtUsername" )).clear();driver.findElement(By.name( "txtUsername" )).sendKeys( "LoginSuccess" );driver.findElement(By.name( "txtPassword" )).clear();driver.findElement(By.name( "txtPassword" )).sendKeys( "1" );try{String html = driver.findElement(By.tagName("body")).getText();Assert.assertEquals( true, html.toLowerCase().contains("search page"));}catch( Exception e ){assertTrue( false );}}}}@Test 
@Order(6) 
